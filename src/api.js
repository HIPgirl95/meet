...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://sdr8heim4d.execute-api.us-east-2.amazonaws.com/dev/api/get-events" +
      "/" +
      token;
...
const getToken = async (code) => {
  ...
  const response = await fetch(
    "https://sdr8heim4d.execute-api.us-east-2.amazonaws.com/dev/api/token" +
      "/" +
      encodeCode
  );
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const response = await fetch(
        "https://blduih5lmjnk554ptv3xqzbarm0mdtvn.lambda-url.us-east-2.on.aws/api/get-auth-url"
      );
      const result = await response.json();
